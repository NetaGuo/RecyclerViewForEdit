...
import com.example.guoying.editrecyclerview.PhotosPickerActivity;
...
public class PhotoAdapter extends BaseAdapter {
    ...
    //照片选择模式，默认单选
    private int mSelectMode = PhotosPickerActivity.MODE_SINGLE;
    //图片选择数量
    private int mMaxNum = PhotosPickerActivity.DEFAULT_NUM;

    private View.OnClickListener mOnPhotoClick;
    private PhotoClickCallBack mCallBack;
    private String intoType;

    public PhotoAdapter(Context context, List<Photo> mDatas, String type) {
        this.mDatas = mDatas;
    ...
    public void setmSelectedPhotos(List<String> choosedList){
        if (null!=mSelectedPhotos && mSelectedPhotos.size()>0){
            mSelectedPhotos.clear();
        }else{
            mSelectedPhotos = new ArrayList<String>();
        }
        this.mSelectedPhotos.addAll(choosedList);
    }
    ...
    public void setSelectMode(int selectMode) {
        ...
        if(mSelectMode == PhotosPickerActivity.MODE_MULTI) {
            initMultiMode();
        }
    }
    ...
    private void initMultiMode() {
        if (CGlobal.isFirstInPiactureChoose){
            mSelectedPhotos = new ArrayList<String>();
        ...
        mOnPhotoClick = new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!TextUtils.isEmpty(intoType) && "Found".equals(intoType)){
                    int width= Luban.get(mContext).getImageSize(path)[0];
                    int height= Luban.get(mContext).getImageSize(path)[1];
                    if ((float)width/height > 5){
                        RegexUtil.showInfo(mContext,"此图过宽，请换张图吧",false);
                        return;
                    }
                    if ((float)height/width >5){
                        RegexUtil.showInfo(mContext,"此图过长，请换张图吧",false);
                        return;
                    }
                }
...
//            Logger.init("PhotoAdapter").hideThreadInfo().setLogLevel(LogLevel.NONE).setMethodCount(3);
...
//            LogUtil.e("图片的位置=="+photo.getPath());
            FoundImageHelper.getInstance().display(photo.getPath(), holder.photoImageView,
                    mWidth, mWidth);
        }
        return convertView;
    }
