...
public class FoundImageHelper {
    ...
    private static FoundImageHelper mInstance;
    private int mWidth;
    ...
    public static synchronized FoundImageHelper getInstance() {
        if(mInstance == null) {
            mInstance = new FoundImageHelper();
        }
        return mInstance;
    }
    ...
    private void initDiskCache() {
        try {
            File cacheDir = OtherUtils.getDiskCacheDir(ScreenManager.getContext(), "images");
            if (!cacheDir.exists()) {
                cacheDir.mkdirs();
            }
            mDiskLruCache = DiskLruCache.open(cacheDir, OtherUtils.getAppVersion(ScreenManager.getContext()),
                    1, 15 * 1024 * 1024);
        } catch (IOException e) {
            e.printStackTrace();
        }
